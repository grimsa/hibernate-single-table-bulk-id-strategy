...
public class SingleGlobalTemporaryTableBulkIdStrategyTest extends AbstractSingleGlobalTemporaryTableBulkIdStrategyTest {
    ...
    public void testDelete() {
        ...
        verify(sqlLog -> sqlLog.contains("delete from Human where (human_id) IN (select ID from HT_TEMP_IDS where ENTITY_NAME='model.TestEntities$Mammal')"));
    ...
    public void testDeleteCleansUpRows_ManyToMany() {
        ...
        verify(sqlLog -> sqlLog.contains("delete from Human_Dog where (Human_human_id) IN (select ID from HT_TEMP_IDS where ENTITY_NAME='model.TestEntities$Human')"));
        verify(sqlLog -> sqlLog.contains("delete from Human where (human_id) IN (select ID from HT_TEMP_IDS where ENTITY_NAME='model.TestEntities$Human')"));
        verify(sqlLog -> sqlLog.contains("delete from Mammal where (id) IN (select ID from HT_TEMP_IDS where ENTITY_NAME='model.TestEntities$Human')"));
        verify(sqlLog -> sqlLog.contains("delete from Animal where (id) IN (select ID from HT_TEMP_IDS where ENTITY_NAME='model.TestEntities$Human')"));
        verify(sqlLog -> sqlLog.size() == 5);
    }
